# Business Envisioning Workshop Checklist

## Document Information

**Project Name**: [Enter AI Project Name]  
**Workshop Date**: [DD/MM/YYYY]  
**Workshop Format**: [Full Day / Two Half-Days / Virtual]  
**Lead Facilitator**: [Name]  
**Date Prepared**: [DD/MM/YYYY]

---

## Checklist Overview

**Purpose**: Ensure comprehensive preparation, smooth execution, and effective follow-through for Business Envisioning Workshops

**How to Use:**
- Check off items as completed
- Update dates in the "Date Completed" column
- Add notes in the "Notes/Owner" column for accountability
- Review weekly leading up to workshop
- Use as run-of-show during workshop day

---

## 4 WEEKS BEFORE WORKSHOP

### Stakeholder Alignment

- [ ] **Executive sponsor identified and committed**
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Workshop objectives aligned with sponsor**
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Initial 1-on-1 meetings scheduled with key stakeholders (30 min each)**
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Participant list drafted and reviewed**
  - Target: 8-15 participants
  - Mix of executive, business, technical, and operational leaders
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

### Initial Planning

- [ ] **Workshop format determined (full day vs. half-days, in-person vs. virtual)**
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Primary and backup dates identified**
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Co-facilitators identified (technical expert, scribe)**
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Budget approved for workshop**
  - Facilitators' time
  - Venue/catering (if in-person)
  - Materials and tools
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

---

## 3 WEEKS BEFORE WORKSHOP

### Stakeholder Discovery

- [ ] **1-on-1 pre-interviews completed with key stakeholders**
  - Executive sponsor
  - Business leaders (minimum 3)
  - Technical leaders (minimum 2)
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Pre-interview insights synthesized**
  - Key themes identified
  - Pain points documented
  - Potential use cases noted
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

### Logistics Setup

- [ ] **Workshop date finalized and confirmed with all participants**
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Calendar invitations sent with:**
  - Workshop date, time, and duration
  - Location (or meeting link for virtual)
  - High-level agenda
  - Objectives and expected outcomes
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Venue booked (for in-person)**
  - Room capacity: Minimum 15 people
  - Natural light preferred
  - U-shape or round table setup
  - Wall space for flip charts
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Virtual meeting setup (for virtual)**
  - Teams/Zoom room scheduled
  - Waiting room disabled for core team
  - Recording enabled (with participant consent)
  - Breakout rooms configured
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Digital collaboration tool selected and set up**
  - [ ] Miro board created and configured
  - [ ] Mural board created and configured
  - [ ] Microsoft Whiteboard created
  - [ ] Other: ___________
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

---

## 2 WEEKS BEFORE WORKSHOP

### Content Preparation

- [ ] **Workshop agenda finalized**
  - Timing allocated for each activity
  - Breaks scheduled
  - Flexibility built in
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Pre-read materials prepared**
  - AI art-of-the-possible overview (3-5 pages)
  - Industry case studies (2-3 relevant examples)
  - Strategic context document
  - Glossary of AI terms
  - **Competitive intelligence brief** (who's doing what in AI)
  - **Industry analyst reports summary** (market trends)
  - **One-page executive summary** of AI opportunity
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Pre-work assignment created and customized**
  - Using template: 13-business-envisioning-pre-work.md
  - Customized for organization
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Workshop templates prepared**
  - Vision statement framework
  - Problem statement template
  - Five Whys root cause analysis template
  - Use case template
  - Prioritization matrix
  - Success criteria template
  - **ROI estimation framework** (TCO, payback period, IRR)
  - **Business case rapid prototype template**
  - **Risk assessment matrix** (technical, business, regulatory)
  - **AI maturity assessment framework**
  - **Workforce impact analysis template**
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Presentation deck created**
  - Welcome and introductions
  - Strategic context
  - AI capabilities overview
  - Industry examples
  - Framework explanations
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Digital collaboration board set up**
  - Pre-populated with templates
  - Frames for each activity
  - Participant access tested
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

### Materials and Supplies

- [ ] **Physical materials ordered (for in-person)**
  - [ ] Sticky notes (multiple colors) - 20 pads
  - [ ] Markers (thick) - 20 count
  - [ ] Flip chart paper - 50 sheets
  - [ ] Tape/adhesive
  - [ ] Printed agenda (one per participant)
  - [ ] Printed templates (backup)
  - [ ] Name tents/tags
  - [ ] Pens/notebooks
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Catering arranged (for in-person)**
  - [ ] Coffee/tea for arrival
  - [ ] Morning break snacks
  - [ ] Lunch (dietary restrictions accommodated)
  - [ ] Afternoon break snacks
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

### Communication

- [ ] **Pre-read materials sent to participants (2 weeks before)**
  - AI overview document
  - Industry case studies
  - Strategic context
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Pre-work assignment sent to participants (2 weeks before)**
  - Clear submission deadline (3 days before workshop)
  - Submission instructions
  - Contact for questions
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

---

## 1 WEEK BEFORE WORKSHOP

### Pre-Work Review

- [ ] **Pre-work submissions tracked**
  - [ ] Submission rate: ____% (target: 80%)
  - [ ] Follow up with non-responders
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Pre-work responses analyzed**
  - Themes identified
  - Common pain points noted
  - Use case ideas captured
  - Concerns flagged
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Agenda refined based on pre-work insights**
  - Specific problems to deep-dive
  - Use cases to explore
  - Concerns to address
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

### Final Logistics

- [ ] **Participant confirmation complete**
  - [ ] Confirmed attendance: ____%
  - [ ] Replacements identified for declines
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Technology tested (for virtual)**
  - [ ] Meeting link tested
  - [ ] Screen sharing tested
  - [ ] Breakout rooms tested
  - [ ] Miro/Mural access tested with sample users
  - [ ] Recording capability tested
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Venue confirmed (for in-person)**
  - [ ] Room setup confirmed (U-shape/rounds)
  - [ ] A/V equipment tested (projector, screens, microphones)
  - [ ] WiFi access tested
  - [ ] Backup room identified (if needed)
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

### Final Preparation

- [ ] **Facilitator prep session held**
  - Lead, technical expert, and scribe aligned
  - Roles and responsibilities clear
  - Backup plans discussed
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Detailed facilitation script reviewed**
  - Talking points for each activity
  - Time allocations confirmed
  - Transition statements prepared
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Participant prep email sent (5 days before)**
  - Reminder of date, time, location/link
  - What to bring/prepare
  - Final agenda
  - Parking information (if in-person)
  - Pre-read reminder
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

---

## 2-3 DAYS BEFORE WORKSHOP

### Final Touches

- [ ] **All pre-work submissions received and reviewed**
  - Summary document created
  - Key insights highlighted for workshop
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Presentation deck finalized**
  - Pre-work insights incorporated
  - Examples customized
  - Final review complete
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Digital board finalized and tested**
  - All frames complete
  - Access permissions set
  - Test run completed
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Backup plans prepared**
  - Printed materials available
  - Offline collaboration options
  - Technical support contact ready
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Facilitator guide printed/accessible**
  - Detailed run-of-show
  - Time allocations
  - Facilitation tips
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

---

## DAY BEFORE WORKSHOP

### Final Checks

- [ ] **Reminder sent to all participants**
  - Date, time, location/link
  - What to bring
  - Parking/building access
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Room setup confirmed (for in-person)**
  - Tables/chairs arranged
  - A/V equipment ready
  - Materials staged
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Virtual setup tested again**
  - Meeting link works
  - All tools accessible
  - Co-facilitators have co-host access
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Catering confirmed (for in-person)**
  - Delivery times confirmed
  - Dietary restrictions accommodated
  - Setup arrangements clear
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

- [ ] **Facilitator team final sync**
  - Roles confirmed
  - Last-minute questions addressed
  - Energy and enthusiasm high!
  - **Date Completed:** ___________
  - **Notes/Owner:** ___________

---

## WORKSHOP DAY - PRE-SESSION

### Setup (60-90 minutes before start)

**For In-Person:**
- [ ] **Arrive early and set up room**
  - **Time Completed:** ___________

- [ ] **Test all A/V equipment**
  - Projector/screens working
  - Microphones working
  - Internet connection strong
  - **Time Completed:** ___________

- [ ] **Materials staged**
  - Name tents at each seat
  - Agendas distributed
  - Sticky notes and markers at tables
  - Flip charts positioned
  - **Time Completed:** ___________

- [ ] **Welcome area set up**
  - Sign-in sheet
  - Name tags
  - Coffee/tea ready
  - **Time Completed:** ___________

**For Virtual:**
- [ ] **Log in 30 minutes early**
  - **Time Completed:** ___________

- [ ] **Test all functionality**
  - Video and audio
  - Screen sharing
  - Digital whiteboard access
  - Breakout rooms
  - Recording
  - **Time Completed:** ___________

- [ ] **Welcome slide displayed**
  - **Time Completed:** ___________

- [ ] **Co-facilitators in room and ready**
  - **Time Completed:** ___________

### Final Prep (15 minutes before start)

- [ ] **Facilitator team huddle**
  - Roles confirmed
  - First few activities reviewed
  - Confidence high
  - **Time Completed:** ___________

- [ ] **Welcome participants as they arrive**
  - Greet warmly
  - Invite to grab coffee/snacks
  - Light conversation
  - **Time Completed:** ___________

---

## WORKSHOP DAY - DURING SESSION

### Activity 1: Welcome & Introductions (30 min)

- [ ] **Start on time**
- [ ] **Welcome and set tone**
- [ ] **Introductions completed**
- [ ] **Agenda reviewed**
- [ ] **Ground rules established**
- [ ] **Ice breaker conducted**
- [ ] **Parking lot set up**

**Notes:**
___________________________________________

### Activity 2: Strategic Context & Vision (60 min)

- [ ] **Executive sponsor shares strategic context**
- [ ] **AI art-of-the-possible presented**
- [ ] **Industry examples shared**
- [ ] **Vision statement co-created**
- [ ] **Vision validated and approved**

**Vision Statement:**
___________________________________________

**Notes:**
___________________________________________

### Activity 3: Business Problem Exploration (75 min)

- [ ] **Problem definition framework explained**
- [ ] **Silent brainstorm completed**
- [ ] **Gallery walk and clustering done**
- [ ] **Top 3-5 problems selected**
- [ ] **Five Whys root cause analysis completed for each problem**
  - Problem 1 root cause documented
  - Problem 2 root cause documented
  - Problem 3 root cause documented
  - Problem 4 root cause documented (if applicable)
  - Problem 5 root cause documented (if applicable)
- [ ] **Deep dive discussions completed for each**
- [ ] **Impact quantified**

**Top Problems Identified (with Root Causes):**
1. Problem: ___________________________________________ 
   Root Cause: ___________________________________________
2. Problem: ___________________________________________ 
   Root Cause: ___________________________________________
3. Problem: ___________________________________________ 
   Root Cause: ___________________________________________
4. Problem: ___________________________________________ 
   Root Cause: ___________________________________________
5. Problem: ___________________________________________ 
   Root Cause: ___________________________________________

### Break (15 min)

- [ ] **Break taken on schedule**

### Activity 4: AI Use Case Ideation (90 min)

- [ ] **AI capability primer delivered**
- [ ] **Structured ideation completed**
- [ ] **10-15 use cases selected for development**
- [ ] **Small groups developed use cases**
- [ ] **Each use case validated against root causes from Five Whys**
- [ ] **Gallery walk and share completed**

**Number of Use Cases Generated:** _______

**Top Use Cases Developed (with Root Cause Alignment):**
1. Use Case: ___________________________________________ 
   Addresses Root Cause: ___________________________________________
2. Use Case: ___________________________________________ 
   Addresses Root Cause: ___________________________________________
3. ___________________________________________
4. ___________________________________________
5. ___________________________________________

### Lunch Break (60 min)

- [ ] **Lunch on schedule**

### Activity 5: Use Case Prioritization (60 min)

- [ ] **Prioritization framework explained**
- [ ] **Individual scoring completed**
- [ ] **Consensus scoring achieved**
- [ ] **2x2 matrix plotted**
- [ ] **Roadmap sequenced**

**Prioritized Use Cases:**
1. ___________________________________________
2. ___________________________________________
3. ___________________________________________

### Activity 6: Success Criteria & Value Definition (60 min)

- [ ] **Success metrics framework explained**
- [ ] **Success criteria defined for top use cases**
- [ ] **Value realization framework created**
- [ ] **Measurement plan established**
- [ ] **ROI projections developed** (cost savings, revenue impact, efficiency gains)
- [ ] **Financial scenarios created** (best case, likely case, conservative case)

**Key Success Metrics:**
___________________________________________

**ROI Summary:**
- Projected ROI: ___________
- Payback Period: ___________
- NPV/IRR: ___________

### Break (15 min)

- [ ] **Break taken on schedule**

### Activity 6A: Executive Decision Points & Risk Governance (30 min)

- [ ] **Investment thresholds discussed**
  - Budget appetite confirmed
  - Resource allocation priorities
  - Speed vs. scale trade-offs
- [ ] **Risk & governance framework addressed**
  - AI ethics and responsible AI principles
  - Regulatory compliance requirements (GDPR, industry-specific)
  - Data privacy and security considerations
  - Model bias and fairness evaluation
  - Reputational risk assessment
- [ ] **Change management implications discussed**
  - Workforce transformation needs
  - Skills gap analysis
  - Cultural readiness assessment
  - Stakeholder communication strategy
- [ ] **Strategic options evaluated**
  - Build vs. buy vs. partner decisions
  - Ecosystem partnership opportunities
  - Infrastructure prerequisites

**Investment Decisions:**
- Approved Budget Range: ___________
- Resource Commitment: ___________
- Risk Tolerance Level: ___________

**Key Governance Decisions:**
1. ___________________________________________
2. ___________________________________________
3. ___________________________________________

### Activity 7: Feasibility & Constraints (45 min)

- [ ] **Feasibility dimensions explained**
- [ ] **Constraints identified for each use case**
- [ ] **Risks documented**
- [ ] **Go/No-Go decisions made**
- [ ] **AI maturity assessment conducted**
  - Current state vs. required state
  - Gap analysis
  - Maturity roadmap milestones
- [ ] **Competitive positioning evaluated**
  - First-mover advantage opportunities
  - Threat analysis (cost of inaction)
  - Market disruption scenarios

**Feasibility Assessment Summary:**
___________________________________________

**Maturity Assessment:**
- Current AI Maturity Level: ___________
- Target Maturity Level: ___________
- Key Gaps: ___________________________________________

### Activity 8: Roadmap & Next Steps (30 min)

- [ ] **Workshop synthesis presented**
- [ ] **High-level roadmap reviewed**
- [ ] **Immediate next steps defined**
- [ ] **Action items assigned with owners and dates**
- [ ] **Closing and appreciation**

**Key Next Steps:**
1. ___________________________________________
2. ___________________________________________
3. ___________________________________________

### Post-Workshop Immediate

- [ ] **Take photos of all flip charts/whiteboards (in-person)**
- [ ] **Save/export all digital collaboration boards**
- [ ] **Collect all sticky notes and artifacts**
- [ ] **Gather feedback (informal check-out)**
- [ ] **Facilitator team debrief (15 min)**
  - What went well
  - What to improve
  - Key insights
  - Immediate action items

**Debrief Notes:**
___________________________________________

---

## POST-WORKSHOP - WITHIN 24 HOURS

### Documentation

- [ ] **Upload all photos and digital artifacts to shared folder**
  - **Date Completed:** ___________
  - **Location:** ___________

- [ ] **Transcribe key outputs**
  - Vision statement
  - Problem statements
  - Use case summaries
  - Prioritization results
  - **Date Completed:** ___________

- [ ] **Create action item tracker**
  - All actions captured
  - Owners assigned
  - Due dates set
  - **Date Completed:** ___________

### Communication

- [ ] **Send thank you email to participants**
  - Appreciation for participation
  - Quick summary of accomplishments
  - Timeline for detailed report
  - **Date Completed:** ___________

- [ ] **Inform executive sponsor of outcomes**
  - Brief summary
  - Key decisions made
  - Next steps
  - **Date Completed:** ___________

---

## POST-WORKSHOP - WITHIN 3 DAYS

### Deliverables

- [ ] **Comprehensive workshop summary report created**
  - Executive summary (1 page)
  - Vision statement and strategic context
  - Business problems (detailed)
  - Use case catalog (all ideas)
  - Prioritized use cases (detailed)
  - Success criteria and value framework
  - Feasibility assessment
  - Roadmap and action plan
  - Appendices
  - **Date Completed:** ___________

- [ ] **Executive decision memo created** (1-2 pages)
  - Using template: 15-executive-decision-memo.md
  - Investment recommendations
  - ROI projections and financial scenarios
  - Risk summary and mitigation strategies
  - Strategic rationale
  - Go/No-Go recommendation
  - **Date Completed:** ___________

- [ ] **Board presentation deck created** (if applicable)
  - Strategic context and competitive landscape
  - Investment opportunity summary
  - Financial projections and ROI
  - Risk and governance framework
  - Approval request
  - **Date Completed:** ___________

- [ ] **90-day action plan created**
  - Immediate next steps with resource requirements
  - Quick win identification
  - Milestone tracking approach
  - **Date Completed:** ___________

- [ ] **Executive dashboard mockup created**
  - Key metrics to track
  - Reporting frequency
  - Visualization format
  - **Date Completed:** ___________

- [ ] **Use case documents created for top 3-5 use cases**
  - Using template: 14-business-envisioning-use-case-template.md
  - All sections completed
  - **Date Completed:** ___________

- [ ] **Initial roadmap document created**
  - Phases defined
  - Milestones identified
  - Timeline estimated
  - **Date Completed:** ___________

- [ ] **Presentation deck created for leadership**
  - Key findings
  - Recommendations
  - Next steps
  - **Date Completed:** ___________

### Distribution

- [ ] **Workshop summary distributed to all participants**
  - **Date Completed:** ___________
  - **Method:** ___________

- [ ] **Request feedback via survey**
  - Workshop effectiveness
  - Facilitator performance
  - Suggestions for improvement
  - **Date Completed:** ___________

---

## POST-WORKSHOP - WITHIN 1 WEEK

### Validation

- [ ] **Financial estimates validated with finance team**
  - Cost savings realistic
  - Revenue projections reasonable
  - ROI calculations reviewed
  - **Date Completed:** ___________

- [ ] **Technical feasibility confirmed with IT/architecture**
  - Technology options assessed
  - Integration complexity validated
  - Timeline realistic
  - **Date Completed:** ___________

### Planning

- [ ] **Data assessment initiated**
  - Data availability checked
  - Data quality evaluated
  - Data gaps identified
  - **Date Completed:** ___________

- [ ] **Project charter or business case drafted**
  - Using workshop outputs
  - Financial analysis included
  - Approval path identified
  - **Date Completed:** ___________

- [ ] **Project governance structure defined**
  - Steering committee formed
  - Meeting cadence set
  - Decision-making process clear
  - **Date Completed:** ___________

- [ ] **Team members identified for discovery phase**
  - Roles defined
  - Candidates identified
  - Availability confirmed
  - **Date Completed:** ___________

---

## POST-WORKSHOP - WITHIN 2 WEEKS

### Socialization

- [ ] **Executive leadership presentation delivered**
  - Findings presented
  - Questions answered
  - Feedback incorporated
  - **Date Completed:** ___________

- [ ] **Formal approval secured**
  - [ ] Executive sponsor approval
  - [ ] Budget approval
  - [ ] Resource allocation approval
  - **Date Completed:** ___________

### Launch

- [ ] **Project initiation announced**
  - Communication plan executed
  - Stakeholders informed
  - Excitement generated
  - **Date Completed:** ___________

- [ ] **Discovery phase kickoff scheduled**
  - Team assembled
  - Kickoff meeting scheduled
  - Initial activities planned
  - **Date Completed:** ___________

- [ ] **Quick win identified and initiated (if applicable)**
  - Smallest, highest-value use case
  - 30-60 day timeline
  - Build momentum and confidence
  - **Date Completed:** ___________

---

## POST-WORKSHOP - WITHIN 4 WEEKS

### Transition

- [ ] **Discovery phase launched**
  - Team mobilized
  - Activities underway
  - Progress tracking established
  - **Date Completed:** ___________

- [ ] **First steering committee meeting held**
  - Governance activated
  - Progress reviewed
  - Issues addressed
  - **Date Completed:** ___________

- [ ] **Value tracking framework implemented**
  - Baseline metrics captured
  - Tracking mechanisms established
  - Reporting schedule set
  - **Date Completed:** ___________

---

## CONTINUOUS IMPROVEMENT

### Lessons Learned

**What Went Well:**
1. ___________________________________________
2. ___________________________________________
3. ___________________________________________

**What Could Be Improved:**
1. ___________________________________________
2. ___________________________________________
3. ___________________________________________

**Five Whys Technique Effectiveness:**
- [ ] Helped uncover root causes effectively
- [ ] Participants understood and engaged with the technique
- [ ] Led to better-aligned AI solutions
- **Notes:** ___________________________________________

**Changes for Next Workshop:**
1. ___________________________________________
2. ___________________________________________
3. ___________________________________________

### Metrics Captured

**Participant Feedback:**
- Average satisfaction score: _____ / 5.0
- Would recommend workshop: ____%
- Felt time was well spent: ____%

**Output Quality:**
- Number of use cases generated: _____
- Number of use cases prioritized: _____
- Vision statement approved: Yes / No
- Roadmap created: Yes / No

**Follow-Through:**
- Summary distributed on time: Yes / No
- Executive approval within 2 weeks: Yes / No
- Project initiated within 4 weeks: Yes / No

---

## SIGN-OFF

**Workshop Preparation Reviewed By:**
- **Lead Facilitator:** _____________________ Date: _______
- **Technical Expert:** _____________________ Date: _______
- **Executive Sponsor:** _____________________ Date: _______

**Workshop Successfully Completed:**
- **Lead Facilitator:** _____________________ Date: _______
- **Project Sponsor:** _____________________ Date: _______

---

**Document Version:** 1.0  
**Last Updated:** January 9, 2026  
**Owner:** [Lead Facilitator Name]

---

*This checklist is part of the AI Delivery Methodology. For related materials, see:*
- *[Business Envisioning Workshop Guide](../guides/01-business-envisioning-workshop-guide.md)*
- *[Pre-Work Assignment Template](../templates/13-business-envisioning-pre-work.md)*
- *[Use Case Template](../templates/14-business-envisioning-use-case-template.md)*
